...
simple_expr :: { Expr AlexPosn }
            : '(' expr ')' {$2}
            | '[' rev_enum_sep(expr,',') ']' {EList (fst $1) (reverse $2)}
...
expr :: { Expr AlexPosn}
     ...
     | '-' expr                                                                                   {EUnaryMinus (fst $1) $2 }
     | expr '-' expr                                                                              {EApp (fst $2) (EApp (fst $2) (EOp (fst $2) Minus) $1) $3 }
     | expr op expr                                                                               {EApp (fst $2) (EApp (fst $2) ((\(pos,TBinOp x) -> EOp pos x) $2) $1) $3 }
     | if expr then expr else expr                                                                {EITE (fst $1) $2 $4 $6 }
     | let liaisons in expr                                                                       {ELet (fst $1) $2 $4}
     | case expr of '{' '[' ']' '->' expr ';' ident1 ':' ident1 '->' expr option(';') '}'         { ECase (fst $1) $2 $8 ((\(_,TIdent1 s) -> s) $10 , (\(_,TIdent1 s) -> s) $12 , $14) } 
     | expr ':' expr                                                                              {EApp (fst $2) (EApp (fst $2) (EOp (fst $2) Cons) $1) $3}
     | do '{' rev_enum_sep1(expr,';') option(';') '}'                                             {EDo (fst $1) (reverse $3)}
     | return '(' ')'                                                                             {Return (fst $1)} 
